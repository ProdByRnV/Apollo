...
### The host harness (Phase 10e-1)

Until 10e-1, nothing in Apollo had been loaded into a host. `ApolloHostTests`
(`Tests/Host/`) loads the VST3 bundle the build produced through JUCE's VST3
hosting implementation and drives it the way a DAW does, through the VST3
interfaces and the wrapper Apollo did not write (ADR-0075). It links no Apollo
code — it knows Apollo only from the bundle and from the parameter registry — is
headless, and runs in CI on all three platforms, after the plugin build.

Four test classes, 159 assertions, all passing:

| Class | Covers |
|---|---|
| Host discovery and loading | A scan finds one class, an instrument, named Apollo by ProdByRnV at 0.1.0, identical across two scans; an instance has stereo out, no active input, no latency and a tail of its release; **forty load-prepare-play-unload cycles** and **sixteen instances at once**, each obeying only its own parameters; every registry parameter visible to the host exactly once and by a unique name; **every VST3 parameter ID pinned** and checked twice — that it still finds its own parameter, and that it is still JUCE's hash of the string ID; every default, step count and automation flag as the registry states it, with exactly the four MIDI expression settings non-automatable; mono and stereo output accepted and sounding, 5.1 refused |
| Host state, presets and automation | A project saved and reopened restores every parameter **as the host reads it back** and plays sample-identically; state restored before the first prepare survives it; state restored mid-note replaces the sound without a non-finite sample; **a host's own `.vstpreset`** written and read by another instance; random bytes, nothing, and another plugin's XML each refused with nothing moved; a recalled chain reports its latency on prepare, before any audio; host automation reaches the sound; and a stepped master-gain lane arrives as a ramp |
| Host processing | Notes start at the same offset wherever in a block the host puts them; a note-off reaches exact silence; **the sustain pedal, pitch bend and All Notes Off arrive as parameters through `IMidiMapping`** and work; blocks of 0 to 512 samples are sample-identical to fixed blocks; one instance re-prepared at 44.1, 96, 22.05, 192, 88.2 and 48 kHz is in tune at each; **offline at 8192-sample blocks is sample-identical to realtime**; the reported tail ends exactly where the release does; a synced delay follows the host's tempo, playing or stopped, and falls back to 120 BPM with no transport or no tempo; and **the reported latency equals the measured one**, follows the rack, and returns to zero |
| Host bypass | Silent while bypassed and playing again after; **a note released during the bypass does not come back**; **a sustain pedal lifted during it is lifted**; a reverb tail frozen at the bypass does not resume; latency unchanged by bypassing |

**What it found.** Each of these was shown failing against the build from
before its fix, with `--plugin` pointed at the older bundle:

| Defect | Since | Fix |
|---|---|---|
| **All 99 integer parameters published to VST3 hosts as continuous** — selectors drawn as knobs, automation landing between values | Phase 2 | `DiscreteIntParameter`: `juce::AudioParameterInt` does not override `isDiscrete()`, and the wrapper publishes a step count only for a discrete parameter |
| **Mono output refused by every VST3 host** | Phase 1 | VST3 describes an inactive input as its last layout, so mono-out arrived as stereo-in; the rule that the input could not be wider than the output guarded a pass-through Apollo no longer does, and is gone |
| **A note or sustain pedal released during a host bypass never ended**, and a reverb tail frozen at the bypass resumed after it | Phase 3 (the tail, 8c) | `processBlockBypassed`: silent, clears voices and tails as the bypass begins, and applies every MIDI message except a note-on |
| JUCE's default bypass asserts in Debug whenever the rack reports latency | Phase 8a | The same override |

Measured on the way, on the development machine:
...
(§5b). The difference is the wrapper building 2,308 host-side parameters per
instance, and is not Apollo's to remove. The cold figure is the module and the
shared wavetables, paid again by a host that unloads the module when its last
instance closes — which is why the load-and-unload cycle costs what it does.

**What it cannot see.** The editor, and anything a particular host does beyond
...
### The macOS package (Phase 11b)

Built without a Mac, which decides the shape of the sub-phase: settle what is a
decision, build the checks a CI runner can perform on Apollo's behalf, and name
what stays unverified (ADR-0077).

**Two settings were missing, and neither announces itself.**

| | Was | Now |
|---|---|---|
| Deployment target | Unset, so a build demanded the macOS version of the machine that made it | `11.0` — Big Sur, and what the arm64 half of a universal binary requires anyway |
| Architectures | The builder's only, so every Intel Mac would get a plugin that does not appear at all | `APOLLO_MACOS_UNIVERSAL` builds `arm64;x86_64`, off by default, on for the macOS CI job |

**The flag is not the evidence.** The package test reads the built binary and
asserts every architecture the build asked for is really in it, because a flag
that silently did nothing would look exactly like success.

**A binary reader, written where it could be tested.** The check that found the
Visual C++ Redistributable dependency reads a binary's own table of what the
loader must find (ADR-0076). `Tests/Package/BinaryImage.h` now reads PE imports
and Mach-O load commands, thin or universal, choosing by the file's magic
number rather than by the platform running the test — so a macOS bundle can be
inspected from anywhere, which is the only reason this could be written here at
all.

It is tested against binaries assembled byte by byte in
`Tests/Package/BinaryImageTests.cpp`: a universal binary with two slices, a thin
one, a library list, and the malformed cases that turn a parser into a crash —
a fat header claiming thousands of slices, a slice offset past the end of the
file, a load command with a size of zero, a file that stops after its magic
number. They run on every platform, so the Mach-O reader was exercised on
Windows before any Mac saw it.

**What the first green macOS run showed** (run 35901131724):

| | |
|---|---|
| Package | `Apollo-0.1.0-macos-universal.zip` |
| Plugin and standalone | `x86_64+arm64`, read back out of the built file |
| Dependencies | 26, every one a macOS framework or a `/usr/lib` dylib — nothing from Homebrew, `/usr/local` or `@rpath` |
...
**The build tree's bundle was not signed at all**, which `codesign` said plainly
before the packaging step signed it: the linker's ad-hoc signature covers a
Mach-O binary, not the bundle around it. Signing is a packaging step, and on
Apple Silicon an unsigned bundle is one macOS will refuse.

**WebKit is in the dependency list**, which is the WKWebView backend being
linked in. It is the nearest thing to evidence for the editor that can be had
without a Mac, and it is not the same as having seen it render.

**What 11b does not claim:** the editor rendering, Retina scaling, the
standalone ever being launched, Release as a configuration, and Developer ID
signing with notarisation — which needs an Apple developer account, and without
which a downloaded plugin is refused as coming from an unidentified developer.

**Three of the failures on the way were the tests, not the package:** a comma
inside `$<BOOL:...>` that CMake reads as its own argument separator, so macOS
never configured; a Linux check that should have stayed a skip until 11c; and a
manufacturer code looked for in a VST3 `Info.plist`, which is an Audio Unit
convention and is not there.

### The Linux package (Phase 11c)

The same shape as 11b, and it found that the question itself is different on
Linux (ADR-0078).

**The ELF reader.** `Tests/Package/BinaryImage.h` reads `DT_NEEDED` entries —
resolving the string table's virtual address through the `PT_LOAD` segments —
and the architecture from `e_machine`. `DT_RPATH` and `DT_RUNPATH` are recorded
as dependencies in their own right, because a binary carrying a search path
from the machine that built it finds nothing on anybody else's. Tested against
ELF shared objects assembled byte by byte in the test, so it ran on Windows
before a runner saw it.

**Three classes, because Linux has no single answer.** macOS has one —
`/usr/lib` or `/System/Library` — and Windows has one. Linux has a desktop with
GTK, a container with almost nothing, and distributions that disagree about
every version suffix. So each dependency is *always present* (the C and C++
runtimes), a *desktop prerequisite* (named in the installation notes), or
*unexpected* — and only the last fails the test.

**What the binaries actually link** (run 35907239478):

| | Linked | Prerequisites among them |
|---|---:|---|
...
at run time with `dlopen`: `juce_XSymbols_linux.h` holds handles for
`libX11.so.6`, `libXext.so.6`, `libXcursor.so.1`, `libXinerama.so.1`,
`libXrender.so.1` and `libXrandr.so.2`, and `juce_WebBrowserComponent_linux.cpp`
opens `libwebkit2gtk-4.1.so` with `libjavascriptcoregtk-4.1.so` and
`libsoup-3.0.so`, falling back to the 4.0 generation.

**So the dependency check is necessary and not sufficient here.** It proves
nothing unexpected is linked. It cannot enumerate what is opened at run time,
and a package that passed it while missing WebKitGTK would still open an empty
...
install on Debian and Ubuntu, Fedora and Arch. CI then reads the sonames back
out of the **staged** file and checks each against `ldconfig` on a real system,
with the two WebKitGTK generations treated as alternatives. Nothing in a build
would notice a typo in a library nobody links; this does.

**What 11c does not claim:** that anyone has seen the interface on Linux —
WebKitGTK is opened at run time and CI has no display; that the graceful path
without WebKitGTK works, which follows from JUCE gating on `webKitIsAvailable`
and has not been run; or that any distribution other than Ubuntu has been
tried. The Fedora and Arch package names are translations of the same library
list.

### ARM64 (Phase 11d)

A fifth CI job, on an `ubuntu-24.04-arm` runner, which passed on its first
run: the whole suite, 182 host assertions against a staged `aarch64-linux`
package, the fifteen regression renders, and `Apollo-0.1.0-linux-arm64.zip`.
The plugin links seven libraries there and the standalone eight, the same as on
x86-64, `ld-linux-aarch64.so.1` among them (ADR-0079).

**Nothing had to be ported, and that is a dividend rather than luck.** ADR-0073
priced SIMD across unison voices and declined it; ADR-0074 took the data layout
instead. So `Source/` contains no intrinsics, no inline assembly and no `_mm_`
or `__m128` — checked, not assumed — and denormal handling is
`juce::ScopedNoDenormals`, which sets the ARM FPCR as readily as the x86 MXCSR.

**The regression goldens crossed an instruction set for the first time.**
ADR-0068 stored each render as a description of the audio rather than as
samples, arguing that identical source does not produce identical floats across
compilers. That had only ever been tested across three compilers on one
instruction set:
...
**not** bit-identical between platforms — the per-case checksums differ, which
is what FMA contraction on ARM64 against a baseline x86-64 with no FMA would
predict — and every fingerprint matches anyway, by two orders of magnitude.

That is not looseness: §5d's sensitivity test shows the same fingerprints
resolving a 0.06 % change in wavetable position. The gap between what a
compiler does to the last bits and what a change does to the sound is simply
very wide.

One curiosity, recorded because it is true rather than because it means
anything: the `Init` render alone is bit-identical between the two ARM64 jobs,
and between the two x86-64 jobs. Every other case differs on every platform, so
the grouping is not "one result per architecture" and nothing should be built
on it.

**Not covered:** Windows on ARM64 (no runner, and JUCE targets `arm64ec` there,
which is a different target rather than a rebuild); the interface on any ARM64
machine, since CI has no display; and performance, which §35's methodology ties
to one machine's reference kernel and which a runner cannot supply
reproducibly.

### The reliability suite (Phase 12a)

Everything before this asks whether Apollo is correct once. This asks whether
it stays correct — and it is the first suite where the failures found were
mostly its own (ADR-0080).

`ApolloHostTests --category Reliability`, through a real VST3 host, with
`--soak N` to run any case longer. The defaults are about twenty seconds in
total, which is what CI pays on every push.

| Case | What it does |
|---|---|
| A long session | Notes arriving and releasing, a new patch every two seconds, for as long as the scale says. 20 s and 40 notes by default; 120 s and 240 notes at `--soak 6` |
| Repeated load and unload | 25 load-prepare-play-unload cycles by default, 150 at ×6, each instance playing, with the resident footprint measured after settling and at the end |
| Preset changes while playing | 120 state recalls under a sounding voice, through the host's own route |
| Automation every block | Every parameter swept; 11,600 moves by default, 69,600 at ×6, rebuilding the rack hundreds of times a second |
| Repeated device changes | Six sample rates against six block sizes, repeatedly, with the instrument still within half a hertz of 220 Hz after each |
| Malformed state while playing | 150 damaged documents mid-render, then eight refusable ones checked exactly |
| Extreme feedback | Maximum delay feedback, a twenty-second reverb, two resonant filters, a hard clipper and 12 dB of makeup |

**What it asserts.** Finite, every sample of every block. Bounded. **Not
trending upward** — the median window level of the last third against the
first, because a range is the wrong statistic when the patch keeps changing.
Still able to stop: exact silence after the last release, with the rack emptied
so the question is the engine's. Memory comes back. And after corruption, every
parameter still holds a legal value.

**What it found in Apollo: an over-long block was silently skipped.** A block
...
the buffer — so the oversampled effects simply did nothing and the audio came
out unprocessed. It is now processed in pieces of the prepared size, which
produces audio identical to the same span in legal blocks; without the fix that
comparison is out by half of full scale. The test lives against the processor,
because a VST3 wrapper's own buffers are sized to the same promise and corrupt
long before Apollo sees the block.

**What it found in itself, which was more.** Five assertions were wrong before
any of them was right:

| Wrong | What it looked like |
|---|---|
| MIDI placed by `start % 48000 == 0` at 512 samples a block | Most note-ons and **every** note-off silently never fired. One note was held for the whole run, and the tail that followed was that note still playing — indistinguishable from a stuck voice in the engine |
| Exact silence demanded within 50 s of a 0.95-feedback delay | About 830 repeats to the flush point: seven minutes. §5c records making the identical mistake and fixing it by arithmetic |
| Loudest window against quietest, while changing patch every two seconds | A legitimately quiet patch made the ratio enormous |
| "Nothing moved" after arbitrary corruption | A flipped byte inside a value produces a document that is still valid, and a valid document is meant to be applied. Only ×6 ran long enough to land one |
| Exact zero after the hostile patch at ×6 | 2.6e-37, which is -727 dBFS. Asserting the stronger thing made the test fail for running longer |

Two of those first read as defects in Apollo. That is the argument for running
a soak early: it is as likely to find a fault in its own measurement as in the
thing measured, and a suite that cries wolf is worse than none.

**The memory figure, and what it is worth.** Two runs of the identical 145
cycles reported **1.1 MB** and **12.3 MB** of growth, so it cannot prove the
absence of a leak. What it catches is the leak that matters: an instrument is
about 2.6 MB (§5b), so one leaked per cycle would be some 380 MB.

**Not covered:** interface reload inside a host. The headless harness cannot
open an editor; the bridge's attach and detach are covered in
`Tests/UI/ParameterBridgeTests.cpp`, and opening and closing the window was
driven by hand on Windows (§5a).
...
| 11 | **Complete.** 11a (Windows), 11b (macOS), 11c (Linux) and 11d (ARM64): install rules and a CPack archive per platform, seven package tests including the dependency table of both binaries, and the staged package driven through the whole host suite on all four jobs (ADR-0076 to ADR-0079). macOS is universal and ad-hoc signed; Linux names its runtime libraries and CI checks each resolves; ARM64 passed first time. Absent: everything that needs a desktop — the editor rendering on macOS or Linux, Retina — plus Developer ID signing with notarisation, and Windows on ARM64 |
| 12 | **12a (reliability) is done**: a soak suite in the host harness with `--soak N`, which found an over-long block silently skipping the oversampled effects and five wrong assertions of its own (ADR-0080). Absent: 12b audio quality, 12c UX, 12d documentation, 12e the release candidate; and interface reload inside a host, which a headless harness cannot do |
...
**Verified by CI against Phase 11d code** (run 35923865141, all five jobs green).
The matrix is now Windows, macOS, Linux x86-64, **Linux ARM64** and the Linux
Clang sanitizer job. All four build jobs stage the package, run the host harness against the
**staged** bundle, build the archive and keep it; the macOS job builds
`arm64;x86_64` for the whole job, signs the staged bundles ad-hoc and verifies
the signature; the Linux job checks that every library `INSTALL.txt` names
resolves against `ldconfig`. Those archives are RelWithDebInfo builds and exist to prove the
package builds and loads; a release is cut from `Release` (Docs/BUILD.md §7).
...
**Still unverified:** Windows on ARM64 — no runner, and JUCE targets `arm64ec`
there, which is a different target rather than a rebuild (§6, issue 7). Linux
and macOS on ARM64 are both covered (11b, 11d).

---

## 5. Test status

**2,320,423 assertions, 0 failures**, across 41 test classes, in `ApolloTests`; and **214 assertions on Windows, 0 failures**, across 6 classes, in `ApolloHostTests`, which loads the built VST3 through a host (§2, "The host harness") and, pointed at a staged install, checks the package itself (§2, "The Windows package"). Both run in `Release` as well as `RelWithDebInfo`. The table below
...
Passing under `Debug`, `RelWithDebInfo`, and `Release` with warnings as errors. The 10e-1 figures above were run under `RelWithDebInfo`, which is also the configuration every CI job runs.
...
### The standalone after the host fixes, 2026-09-21

10e-1 changed the processor — its bypass, its bus rule and the class of its
integer parameters — so the standalone built from it was launched from the
project root copy: window titled "Apollo", responding after ten seconds, 98 MB
working set, and closed through its window with exit code 0.

That is a smoke check and no more. The standalone never bypasses, uses the
...
### Voice engine after 10d-1 (Phase 10d)

The heaviest column above is the one 10d-1 set out to move, and it moved by
**1.8x**. Measured as three interleaved runs of each binary in one sitting
(ADR-0070), medians reported, normalised:

| Voices | Heaviest, before | Heaviest, after | Ratio |
|---:|---:|---:|---:|
| 1 | 7.15 % | 4.06 % | 1.76x |
| 8 | 57.01 % | 31.62 % | 1.80x |
| 32 | 233.8 % | 130.9 % | 1.79x |

**A caveat about the absolutes, recorded rather than smoothed over.** The
"before" column measures exactly the code 10c measured, and it reads about a
third higher than 10c's published 174.7 % — on *normalised* figures, which are
supposed to transport between sittings. The worst-case callback row transports
almost exactly (below), so whatever is not transporting is specific to this
measurement path rather than to the machine as a whole.

The consequence is a limit on what the harness supports, and it is worth stating
plainly: **a ratio measured within one sitting is sound; a normalised absolute
compared across sittings is not**, at least for this row. 10c's design
anticipated drift within a run and corrected for it, and did not anticipate this.
The 1.8x above is an interleaved same-sitting comparison and is unaffected.

**The heaviest patch still cannot sustain full polyphony** — 131 % rather than
234 %, so the ceiling moved but did not clear. What *did* clear is the thing that
actually mattered, which is the next section.
...
### Worst-case callback after 10d-1 (Phase 10d)

This is the row 10d-1 was for, and **it no longer misses a block**.

Three interleaved runs of each binary in one sitting, medians reported. The
machine sat at 0.829-0.864x the reference across all six runs, so these are
directly comparable without correction. Raw, because the question "does this
keep up" is a question about a real machine rather than a normalised one.

| Worst patch, 32 held + stealing | Median | p99 | p99.9 | Worst |
|---|---:|---:|---:|---:|
| Before 10d-1 | 104.6 % | 121.7 % | 126.1 % | 126.2 % |
| **After 10d-1** | **64.2 %** | **78.3 %** | **79.9 %** | **80.1 %** |
| Ratio | 1.63x | 1.56x | 1.58x | 1.58x |

**Nothing over 100 % remains, at any percentile, including the worst single
block of nine hundred.** The worst patch the controls can build now has a fifth
of its deadline spare where it previously had none.

The whole-callback ratio (1.63x) is smaller than the voice-path ratio (1.79x),
and that is the expected shape rather than a discrepancy: 10d-1 touched the
oscillator read and nothing else, so the rack, the filters and the modulation
matrix are now a larger share of a smaller total.

**This row transports across sittings, and the voice-engine row did not.**
Corrected to the reference machine the "before" median reads 89.8 %, against the
90.37 % 10c published for the same code — agreement within one per cent, on a
measurement taken on a different day. That is what 10c built the harness for,
and it is also why the anomaly noted in the voice-engine section above is
recorded as a caveat rather than dismissed: the harness demonstrably can
transport a figure, so a row that does not is telling us something.

### After 10d-2 (Phase 10d)

10d-2 resolved the rest of the per-sample read out of the per-sample path. The
mip level and the frame position cannot change within a modulation block, yet
every sample was re-deriving two bounds-checked lookups, a vector-of-vectors
indirection, three separate computations of the level's frame size, and the
frame-position clamp and blend weight. A `Wavetable::Reader` now settles all of
it once and the oscillator holds it (ADR-0071).

**This was the cheaper of the two candidates, measured first on purpose**, and
it was enough that SIMD across unison voices is now optional rather than needed.

Interleaved runs of the two binaries in one sitting. The headline pair is the
two runs whose machine speeds matched to within one per cent (1.017x and
1.008x), both among the cleanest of the sitting:

| | 10d-1 | 10d-2 | Ratio |
|---|---:|---:|---:|
| Heaviest patch, 32 voices (raw) | 84.7 % | **54.0 %** | **1.57x** |
| Default patch, 32 voices (raw) | 7.15 % | **4.79 %** | 1.49x |
| Worst patch, median callback | 50.9 % | **36.2 %** | **1.41x** |
| Worst patch, p99 | 55.8 % | **39.1 %** | 1.43x |
| Worst patch, worst single block | 62.3 % | **45.8 %** | 1.36x |

Medians over every clean run agree — 1.62x, 1.44x, 1.50x — before correcting for
10d-2's runs having landed on a machine about 4.6 % faster, which puts the true
figures between those and the pair above.

**Against the pre-10d baseline the voice path is about 2.8x cheaper** (1.79x
then 1.57x), and the worst patch Apollo's own controls can build now sits near a
third of its callback deadline where before 10d it exceeded it.

The default patch improved as well, which was not automatic: `setPosition` on a
unison stack used to walk all sixteen voices whether or not they were sounding,
and with a Reader rebuild attached that would have made the one-voice case
*worse*. It now walks only the active voices, and the voices that become active
later are brought up to date where they become active (ADR-0071).

**Two harness faults surfaced here, and neither is fixed.** They are 10d-3.

- **Normalisation over-corrects this row**, which is the anomaly 10d-1 recorded
  without being able to explain. Across 10d-1's runs the *normalised*
  heaviest-patch figure varied from 83 to 131 while the *raw* figure varied only
  from 84.7 to 104.5: normalisation turned a 23 % spread into 58 %. The
  reference kernel is register arithmetic and the voice engine is memory-bound,
  so when the core clock drops the kernel slows proportionally and the table
  reads do not, and dividing by the kernel's ratio over-corrects. The worst-case
  callback row is reported raw and transports between sittings to within one per
  cent, which is why the tables above are raw and speed-matched rather than
  normalised.
- **The steadiness check samples the wrong part of the run.** Contention is
  ...
  runs minutes later. One run reported **2.7 % contention** and then produced a
  **435 %** worst-case block on a patch whose median was 39 % — with every other
  row degraded too, including the default patch's worst case going from 2.4 % to
  41 %. No code change can do that; something else had the machine, and the
  report said the machine was quiet.

  Until the harness can say this itself, a run is discarded here when its
  **default patch, 8 held** worst case exceeds 5 %. The default patch is cheap
  enough that a large worst case there can only be interference, which makes it
  a witness for the whole run. Four of fourteen runs were discarded by that
  rule, and they are named rather than dropped quietly: 10d-2 passes 2, 4 and
  7, and 10d-1 pass 7 — pass 7 having been disturbed for both binaries, which
  is what an external event looks like.

### The harness after 10d-3 (Phase 10d)

10d-1 recorded that normalised figures did not transport between sittings and
could not say why. 10d-2 quantified it: normalising made a row *less*
reproducible, turning a 23 % raw spread into 58 %. 10d-3 found the cause, and it
was not the one 10d-2 proposed.

**The machine sheds up to 68 % of its clock while a report runs.** The
development laptop is an i7-1255U — a 15 W part with a 1.7 GHz base clock and a
turbo near 4.7 — and nothing in the harness had ever asked what the clock does
*during* a measurement. Every normalised figure was divided by a speed taken in
the opening seconds, which on such a part describes a processor that no longer
exists by the later sections (ADR-0072).

**The fix is to stop measuring a clock the machine cannot hold.** The harness now
runs sustained load until the reference unit stops getting slower — forty-five
seconds on this laptop — and only then assesses and reports. Everything that
follows is taken at a sustainable speed, which is also the speed a user's session
runs at, because a synthesiser is not a burst workload.

| | before 10d-3 | after |
|---|---|---|
| Machine speed, between runs | 0.525 – 1.089 (a factor of two) | **0.929 – 0.944 (1.6 %)** |
| Heaviest patch, 32 voices, between runs | 23 % raw, 58 % normalised | **0.4 %** |

Apollo's most expensive measurement now reproduces to within half a per cent
between runs. **The answer to "why do these numbers not reproduce" was "hold the
clock still", not "choose a better divisor"** — which is worth stating plainly,
because the better divisor is what 10d-3 set out to build.

Three other things landed with it:

- **A second reference kernel, bound by memory** — a dependent chase through a
  2 MB table, the size of the wavetable library, reading four neighbours at each
  stop. Every row now declares which reference it resembles, and the parameter is
  required rather than defaulted so all nine call sites had to state a choice.
- **The drift check itself**, reported every run, which is what found the 68 %.
- **A witness for the callback traces.** The default patch at eight voices is too
  cheap to produce a large worst-case block, so one appearing means something
  else had the machine. 10d-2 applied this rule by hand to discard four runs of
  fourteen; it is now the report's own, and it distinguishes sustained
  interference from an isolated event rather than issuing one verdict.

**What is not established, and the report says so on every run.** That the memory
reference is the *better* divisor for memory-bound rows is reasoned, not
demonstrated. Its premise is measured — the two references decouple, their ratio
moving 24 % across six runs — but confirming the claim needs several
low-contention runs spanning a range of machine speeds, and selecting by the
voice row's own quality left two or three, all at nearly the same speed.

The irony is the honest resolution: the warm-up works by removing the speed
variation the validation would need, and in removing it also removes most of the
reason to care, because the raw figures now reproduce on their own. The mechanism
stays because it is still right for comparing between different machines.

**And a plain statement about the hardware.** A 15 W laptop running an antivirus,
a VPN, a browser and an IDE is not a measurement instrument. During this phase
...
### The flat unison stack (Phase 10d-5)

10d-4 priced a restructured inner loop at 1.39x on a unison stack and decided to
build that rather than SIMD. 10d-5 built it (ADR-0074).

`UnisonOscillator` held sixteen `WavetableOscillator` objects of about a hundred
bytes each, so reading sixteen phases meant sixteen loads a hundred bytes apart
and the gains came through a call into the layout object per voice per sample. It
is now flat — parallel arrays for phase, increment, readers and gains — rendered
as a scalar gather pass and then an arithmetic pass over contiguous arrays.

Interleaved against the pre-10d-5 binary, four passes each, one sitting:
...
**The heaviest patch is 1.28x cheaper.** The prototype predicted 1.39x on a stack
in isolation, and the heaviest patch is two stacks plus a sub, a noise generator,
filters and an envelope — so getting 1.28x across the whole patch says the stacks
dominate it.

**ADR-0071's single-implementation property survived the split**, and that was the
main design risk. `Reader` now exposes `gather` and `interpolate`; `read` *is*
gather-then-interpolate, and the stack calls the same two functions in a different
order. A test demands the two schedules agree **bit-identically**, because there
is no arithmetic difference between them for floating point to round differently.

**Three defects surfaced that review would not have found.** The predicted win
arrived only at 1.20x until the cause was hunted; `Taps` had default member
initialisers, so an array of sixteen value-initialised **640 bytes every sample**
whatever the voice count, costing the default patch a real 31 %; and a residual
5 % on that row turned out to be noise rather than a regression, which is
recorded as such rather than claimed as a fix.

The last of those is the useful one to remember: the 31 % was invisible to a
sixteen-voice stack benchmark and only appeared because the report measures the
instrument's own patches, the default one included.

**Left on the table, priced:** the production stack is still slower than the
prototype, because `Taps` is an array of structures where the prototype used
separate arrays. Structure-of-arrays is what the arithmetic pass would need to
vectorise — and taking it means giving up the one-gather-one-interpolate property
above. Declined on the merits, the same way ADR-0073 declined the float option.
...
| **Cross-platform spread** | Reported per case by every CI job. Measured worst deviation from the checked-in reference: **0.0005 dB** on Windows (MSVC, x86-64), Linux (GCC, x86-64) and Linux (GCC, ARM64), and **0.0029 dB** on macOS (Apple Clang, arm64) — against tolerances of 0.1 dB on levels and 0.2 dB on bands. The renders are **not** bit-identical between platforms, which is the case ADR-0068 designed for, tested across two instruction sets for the first time in 11d (ADR-0079) |
...
| 2 | VST3 has not been loaded in a DAW | Medium | **Loaded through a host; not yet through a DAW.** 10e-1 loads the built bundle through JUCE's VST3 hosting implementation — scanning, instantiating, automating, saving, recalling, bypassing and rendering it through the VST3 interfaces — in CI on all three platforms, and found four defects doing so (issues 19-21, ADR-0075). That is conformance to the interfaces, not a DAW: FL Studio is 10e-2. |
...
| 7 | ~~ARM64 unverified~~ | **Closed** | An `ubuntu-24.04-arm` job joined the matrix in 11d and passed on its first run: the whole suite, the host suite against a staged `aarch64-linux` package, the fifteen regression renders within 0.0005 dB of their references, and an archive of its own. macOS ships arm64 as half of its universal binary (11b). Windows on ARM64 remains untried — no runner, and JUCE targets `arm64ec` there (ADR-0079). |
...
| 13 | The heaviest patch cannot sustain full polyphony in real time | **Fixed** | Measured in Phase 4c, re-measured properly in 10c, and closed across **10d-1, 10d-2 and 10d-5**. 10c stated it in the terms that matter: the worst patch the controls can build, at full polyphony with a full rack and the modulation matrix running, missed its callback deadline one block in a hundred — median 90 %, p99 102 %. It no longer misses one and is no longer close to one. 10d-1 removed eight integer divisions and a duplicated cubic from the per-sample table read (ADR-0070); 10d-2 resolved the rest of the read once per modulation block instead of once per sample (ADR-0071); 10d-5 made the unison stack flat rather than sixteen oscillator objects (ADR-0074). Together the voice path is about **3.6x** cheaper than before Phase 10d — the product of three within-sitting ratios, 1.79x, 1.57x and 1.28x, rather than a comparison across sittings, which §5b explains this harness cannot support. **The heaviest patch in isolation now costs 41 % of one core at 32 voices, against roughly 195 % before Phase 10d**, so the ceiling of every control at once finally fits with headroom above it. ADR-0029 records why the ceilings were published rather than lowered, and that decision now costs nothing. |
...
- **No macOS or Linux desktop.** CI builds, tests, packages, signs and
  archives on Windows, macOS, Linux and ARM64 Linux, which covers a great
  deal; what it cannot do is open a window. Phase 11 showed exactly where the
  line falls: each package is self-contained, loads through a VST3 host and
  passes the whole suite, and **nobody has seen either editor render**.
  Retina, the WebKit and WebKitGTK backends, and Developer ID signing with
  notarisation need the machines themselves — and an Apple developer account —
  so they are named rather than ticked. Windows on ARM64 is untried for the
  same reason: no runner (issue 7).
- **What a Linux binary links is not what it needs** (ADR-0078). JUCE opens
  X11, GTK and WebKitGTK at run time, so the dependency check cannot enumerate
  them. The runtime list lives in `INSTALL.txt` and CI checks every name in it
  resolves, which is the best a machine can do; whether the interface actually
  appears under WebKitGTK is still unobserved.
...
**Phase 12b — audio quality.** The measured half is largely done; 12b is the
listening half, and it is the part this machine is worst equipped for.

1. **What is already measured** (§5c): aliasing at -98.5 dBc worst case,
   THD+N 0.0013 % on the one sine source, exact silence at rest, DC offset at
   -112.7 dBFS, inharmonic content at -121.9 dBc, and a flat window trace over
   a minute of the worst patch. Those are the numbers 12b's aliasing, noise
   floor and gain staging reviews would otherwise be establishing.
2. **What is not.** Nobody has *heard* Apollo. The ten factory sounds and the
   four wavetables are measured and pinned and have never been played to a
   person (issue 16: no MIDI source on this machine). "Final listening tests"
   is the one task on the roadmap that cannot be automated at all.
3. **What can be done here**: transient response as a measurement rather than
   an impression — attack timing against the envelope's own figures, and the
   step response already in §5c; and an aliasing sweep across the whole
   keyboard rather than at the four notes 10a used.
4. **What should be honest about its limits.** A listening test done by
   reading spectra is not a listening test, and saying so is better than
   ticking it.
5. **Carried, none blocking**: issue 6, no allocation or lock detector on the
   audio thread; interface reload inside a host (12a); the structure-of-arrays
   taps declined in ADR-0074; the optional spectrum analyser; the desktop-only
   checks Phase 11 named rather than ticked; and 10e-2/10e-3, deferred at the
...
do not transport between sittings; ratios measured alternately hold even when
contention swings from 1 % to 32 %. A row's own spread has to be read before its
median is believed.
