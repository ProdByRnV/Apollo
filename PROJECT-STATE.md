...
| 4 | Linux CI job fails on Phase 2 code | Medium | Windows, macOS and the sanitizer job pass. **Root cause read from the CI log** (`gh run view --log-failed`): `juce_gui_extra.cpp: fatal error: gtk/gtk.h: No such file or directory`, preceded by `warning: "JUCE_WEB_BROWSER" redefined`. Apollo hand-defined `JUCE_WEB_BROWSER=1` instead of setting JUCE's `NEEDS_WEB_BROWSER` option. On Linux `_juce_link_optional_libraries` reads that property to decide *both* the compile definition and whether to link `juce::pkgconfig_JUCE_BROWSER_LINUX_DEPS` — the only source of the GTK/WebKitGTK include paths. The hand-written define switched the include on while the path was never supplied. Fixed by using `NEEDS_WEB_BROWSER` and removing the manual define. **Unconfirmed until a green Linux run.** |
