...
**2,320,420 assertions, 0 failures**, across 41 test classes, in `ApolloTests`; and **182 assertions on Windows and Linux and 189 on macOS, 0 failures**, across 5 classes, in `ApolloHostTests`, which loads the built VST3 through a host (§2, "The host harness") and, pointed at a staged install, checks the package itself (§2, "The Windows package"). Both run in `Release` as well as `RelWithDebInfo`. The table below
...
**Phase 12a — reliability.** Phase 11 is complete, and 12a is the first work in
a while this machine can do in full.

1. **What it covers**, from ROADMAP: long-duration soak, repeated load and
   unload, preset changes, rapid automation, maximum polyphony, extreme
   modulation and feedback, repeated sample-rate changes, interface reload, and
   malformed state.
2. **Much of it already exists in miniature.** The validation suite holds a
   minute of the worst patch the controls allow (§5c); 10e-1 loads and unloads
   forty instances and runs sixteen at once; the effects chain rearranges
   itself every block for 240 blocks. 12a is those tests made longer and
   meaner, with the failure they are looking for being drift and accumulation
   rather than a wrong value.
3. **The host harness is the right vehicle.** It drives a real VST3 host, so a
   soak can automate parameters, recall state and bypass the way a session
   does, rather than through the processor's own interface.
4. **The one thing to decide first** is what a soak asserts. "Still running" is
   weak; the useful assertions are that the output stays finite and bounded,
   that silence after release is still exact, that latency and tail do not
   drift, and that memory does not grow — the last of which needs a measurement
   this project has only taken once (§5b, first use).
5. **Carried, none blocking**: issue 6, no allocation or lock detector on the
   ...
   optional spectrum analyser; the desktop-only checks Phase 11 named rather
   than ticked; and 10e-2/10e-3, deferred at the developer's direction.
