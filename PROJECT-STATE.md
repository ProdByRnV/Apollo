...
| 4 | Both Linux CI jobs fail on Phase 2 code | Medium | Windows and macOS pass. **Diagnosed from JUCE's source, not from the CI log** (no log access here): `JUCEUtils.cmake` resolves the Linux WebView through `pkg_check_modules(... webkit2gtk-4.x gtk+-x11-3.0)` **without `REQUIRED`**, so a missing module is not a configure error — it yields empty include paths and the build dies later on `gtk/gtk.h`. `libgtk-3-dev` was never named in the dependency list, only inherited transitively. Now named explicitly, the WebKitGTK generation is selected at runtime (4.1 or 4.0), and the install step verifies both pkg-config modules so a miss fails immediately and legibly. The sanitizer job now builds only `ApolloTests` with the WebView off, since nothing it runs uses one. **Unconfirmed until a green run.** |
